...
class TestUMDAc(TestCase):
    ...
    def test_list_bounds(self):
        ...
        eda = UMDAc(size_gen=300, max_iter=1, dead_iter=1, n_variables=n_variables, lower_bound=l_bounds,
    ...
    def test_bounds(self):
        ...
        assert not np.any(umda.pm.pm[1, :] < lower_factor)  # ensure the std has been modified
    ...
    def test_new_generation(self):
        n_variables = 10
        umda = UMDAc(size_gen=100, max_iter=1, dead_iter=0, n_variables=n_variables, alpha=0.5, lower_bound=-60,
                     upper_bound=60)
        benchmarking = ContinuousBenchmarkingCEC14(n_variables)
    ...
    def test_check_generation(self):
        n_vars = 10
        umda = UMDAc(size_gen=100, max_iter=100, dead_iter=10, n_variables=n_vars, alpha=0.5, lower_bound=-60,
                     upper_bound=60)
    ...
    def test_evaluate_solution(self):
        ...
        """
        n_variables = 10
        umda = UMDAc(size_gen=100, max_iter=100, dead_iter=10, n_variables=n_variables, alpha=0.5, lower_bound=-60,
                     upper_bound=60)
