...
class UMDAc:
    ...
    def __init__(self, SIZE_GEN, MAX_ITER, DEAD_ITER, ALPHA, vector, aim, cost_function):
        ...
        :param MAX_ITER: total number of iterations in case that optimum is not yet found. If reached, the
        optimum found is returned
        :param DEAD_ITER: total number of iteration with no better solution found. If reached, the optimum found
        is returned
        :param ALPHA: percentage of the generation tu take, in order to sample from them. The best individuals selection
        :param vector: vector of normal distributions to sample from. It is demanded a pandas table with parameters
        names as columns and 'mu', 'std', 'max' (optional), 'min' (optional) as index of the table.
        :param aim: 'minimize' or 'maximize'. Represents the optimization aim.
        :param cost_function: a callable function implemented by the user, to optimize. The cost function must have as
        an input the dictionary with the individual to check the cost. The dictionary has as keys the names of the
        parameters and as values the values to check.
        """
    ...
    def new_generation(self):
        ...
        :return: a pandas dataframe with individuals.
        """
    ...
    def run(self, output=True):
        """
        Run method
        :return: best cost found, best combination of parameters, history of best values
        """
        not_better = 0
